...
Used by tasks/enrich.py via resolve(); does not commit transactions (caller owns).
──────────────────────────────────────────────────────────────────────────────
"""
...
class EntityResolutionService:
    ...
    def resolve(
        ...
        if highest_score >= self.fuzzy_threshold and best_match:
            logger.info(
                "FUZZY RESOLUTION: %r mapped to %r (score=%s)",
                value,
    ...
    def resolve_entities(
        ...
        """Batch helper (optional); does not commit."""
        out: List[Entity] = []
